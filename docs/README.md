...
The event log from the live demo, showing the core claim in action: the agent calls `get_site_health` and answers from the result rather than guessing.

![Happy path: site health check succeeds](docs/screenshots/happy-path.png)

*A site-health tool call and its result, rendered in the reasoning trace.*
